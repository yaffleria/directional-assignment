...
test.describe('Chart Interactions', () => {
  ...
  test('TC-05: Legend Color Change Works', async ({ page }) => {
    ...
    // Find the chart card
    const card = page.locator('.bg-card').filter({ hasText: 'Top Coffee Brands (Bar)' })
  ...
  test('TC-07: Dual Axis Charts Display', async ({ page }) => {
    ...
    const coffeeChart = page.locator('.bg-card').filter({ hasText: 'Coffee Consumption' })
    const snackChart = page.locator('.bg-card').filter({ hasText: 'Snack Impact' })
